...
class MainActivity : FlutterActivity() {
    ...
    // OutputStream wrapper for Base64 streaming
    private class Base64OutputStream(out: OutputStream, flags: Int) : FilterOutputStream(out) {
        private val encoder = Base64.NO_WRAP
        ...
        private fun flushBufferIfNeeded() {
            val bytes = buffer.toByteArray()
            if (bytes.size >= 4096) { // flush every 4KB
                val encoded = android.util.Base64.encode(bytes, encoder)
                out.write(encoded)
                buffer.reset()
            }
        }
        ...
        override fun flush() {
            ...
            if (remaining.isNotEmpty()) {
                val encoded = android.util.Base64.encode(remaining, encoder)
                out.write(encoded)
                buffer.reset()
            }
            super.flush()
