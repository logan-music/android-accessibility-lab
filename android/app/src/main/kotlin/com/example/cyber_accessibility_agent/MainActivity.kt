...
class MainActivity : FlutterActivity() {
    ...
    // Prevent multiple startService() calls and log spam
    @Volatile
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == PERM_REQUEST_CODE) {
            // User returned from MANAGE_ALL_FILES_ACCESS settings screen — re-evaluate permissions
            val map = currentPermissionMap()
            pendingPermResult?.success(map)
            pendingPermResult = null
    ...
    private fun startAgentService() {
        // ensure only one concurrent attempt to start service
        synchronized(this) {
        ...
        try {
            ...
            // reset flag so future attempts can retry
