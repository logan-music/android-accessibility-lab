...
enum CommandAction {
  ...
  listFiles,
  uploadFile,
  sendTelegram,  // ✅ NEW: dedicated action for Telegram send
  zipFiles,
  deleteFile,
  deleteDir,
  deviceInfo,
...
extension CommandActionExt on CommandAction {
  String get name {
    switch (this) {
      ...
      case CommandAction.uploadFile:
        return 'upload_file';
      case CommandAction.sendTelegram:  // ✅ NEW
  ...
  static CommandAction? fromString(String s) {
    ...
    switch (v) {
      ...
      // ✅ FIX: Separate 'send' from 'upload'
      case 'send':
      case 'send_file':
      case 'send_telegram':
      case 'sendtelegram':
        return CommandAction.sendTelegram;
      case 'zip':
      case 'zip_files':
      case 'archive':
      ...
      case 'rd':
      case 'rmdir':
        return CommandAction.deleteDir;
      case 'device_info':
      case 'info':
      case 'device':
      ...
      case 'read-file':
      case 'read_content':
      case 'read_content_uri':
        return CommandAction.readFile;
      case 'prepare_upload':
      case 'prepareupload':
      case 'prepare-upload':
        return CommandAction.prepareUpload;
      default:
        return null;
    }
  }
...
class CommandParser {
  ...
  static ParseResult parse(Map<String, dynamic> raw, String expectedDeviceId) {
    try {
      ...
      // ✅ FIX: Better legacy command handling
      if (actionRaw == null || actionRaw.trim().isEmpty) {
        ...
        if (legacy != null) {
          ...
          // Also merge derived payload if action was derived
          if (derived['payload'] is Map && payloadRaw == null) {
            raw['payload'] = derived['payload'];
          }
        } else {
      ...
      // ✅ FIX: Merge legacy command text payload after initial payload parse
      ...
      // ✅ FIX: Route to appropriate parser including sendTelegram
      switch (actionEnum) {
        case CommandAction.changeDir:
        ...
        case CommandAction.sendTelegram:  // ✅ NEW
          return _parseSendTelegram(id, deviceId, payload, createdAtRaw);
        case CommandAction.zipFiles:
          return _parseZipFiles(id, deviceId, payload, createdAtRaw);
        case CommandAction.deleteFile:
          return _parseDeleteFile(id, deviceId, payload, createdAtRaw);
        case CommandAction.deleteDir:
          return _parseDeleteDir(id, deviceId, payload, createdAtRaw);
        case CommandAction.deviceInfo:
          return _parseDeviceInfo(id, deviceId, payload, createdAtRaw);
        case CommandAction.ping:
          return _parsePing(id, deviceId, payload, createdAtRaw);
        case CommandAction.readFile:
          return _parseReadFile(id, deviceId, payload, createdAtRaw);
        case CommandAction.prepareUpload:
          return _parsePrepareUpload(id, deviceId, payload, createdAtRaw);
        case CommandAction.raw:
          ...
          final cmd = Command(
            ...
            payload: {'text': payload['value'] ?? payload['text'] ?? ''},
            createdAt: createdAt,
          );
  ...
  static ParseResult _parseListFiles(String id, String deviceId, Map<String, dynamic> payload, dynamic createdAtRaw) {
    // ✅ FIX: Better default path handling
  ...
  static ParseResult _parseUploadFile(String id, String deviceId, Map<String, dynamic> payload, dynamic createdAtRaw) {
    ...
    final bucket = _extractString(payload, ['bucket', 'storage_bucket']) ?? 'device-uploads';  // ✅ FIX: Default bucket
    ...
    final cmd = Command(
      ...
      payload: {
        'path': path,
        'bucket': bucket,  // ✅ Always include bucket
  ...
  // ✅ NEW: Dedicated parser for Telegram send
  static ParseResult _parseSendTelegram(String id, String deviceId, Map<String, dynamic> payload, dynamic createdAtRaw) {
    final path = _extractString(payload, ['path', 'file', 'filepath', 'src']);
    if (path == null || path.isEmpty) return ParseResult.err('send_telegram requires path');
    if (path.length > _maxPathLength) return ParseResult.err('path too long');
    
    // Extract Telegram credentials from payload
    final telegram = payload['telegram'];
    if (telegram == null || telegram is! Map) {
      return ParseResult.err('send_telegram requires telegram config (bot_token, chat_id)');
    }
    
    final telegramMap = Map<String, dynamic>.from(telegram as Map);
    final botToken = telegramMap['bot_token'] ?? telegramMap['token'] ?? telegramMap['botToken'];
    final chatId = telegramMap['chat_id'] ?? telegramMap['chatId'] ?? telegramMap['chat'];
    
    if (botToken == null || chatId == null) {
      return ParseResult.err('send_telegram requires bot_token and chat_id in telegram config');
    }
    ...
    final cmd = Command(
      id: id,
      deviceId: deviceId,
      action: CommandAction.sendTelegram,
      payload: {
        'path': path,
        'telegram': {
          'bot_token': botToken,
  ...
  static ParseResult _parseDeleteDir(String id, String deviceId, Map<String, dynamic> payload, dynamic createdAtRaw) {
    final path = _extractString(payload, ['path', 'dir', 'directory', 'target']);
    if (path == null || path.isEmpty) return ParseResult.err('delete_dir requires path');
    if (path.length > _maxPathLength) return ParseResult.err('path too long');
    final createdAt = _parseDate(createdAtRaw);
    ...
    final cmd = Command(
      id: id,
      deviceId: deviceId,
      action: CommandAction.deleteDir,
      payload: {'path': path},
      createdAt: createdAt,
    );
    return ParseResult.ok(cmd);
  ...
  static ParseResult _parseReadFile(String id, String deviceId, Map<String, dynamic> payload, dynamic createdAtRaw) {
    final path = _extractString(payload, ['path', 'file', 'filepath', 'uri', 'content_uri']);
    if (path == null || path.isEmpty) return ParseResult.err('read_file requires path or uri');
    if (path.length > _maxPathLength) return ParseResult.err('path too long');
    final createdAt = _parseDate(createdAtRaw);
  ...
  static ParseResult _parsePrepareUpload(String id, String deviceId, Map<String, dynamic> payload, dynamic createdAtRaw) {
    final path = _extractString(payload, ['path', 'file', 'filename', 'filepath', 'uri', 'content_uri']);
    if (path == null || path.isEmpty) return ParseResult.err('prepare_upload requires path/uri');
    if (path.length > _maxPathLength) return ParseResult.err('path too long');
    final createdAt = _parseDate(createdAtRaw);
    ...
    final cmd = Command(
      id: id,
      deviceId: deviceId,
      action: CommandAction.prepareUpload,
      payload: {'path': path},
      createdAt: createdAt,
    );
    return ParseResult.ok(cmd);
  ...
  // ✅ FIX: Updated legacy command text parser
  static Map<String, dynamic> _deriveActionFromCommandText(String cmdText) {
    ...
    switch (first.toLowerCase()) {
      ...
      case 'pwd':
        return {'action': 'list_files', 'payload': {'path': ''}};
      case 'list':
      case 'ls':
        return {
          'action': 'list_files',
      ...
      // ✅ FIX: /send now maps to send_telegram
      case 'send':
        return {
      ...
      case 'prepare':
      case 'prepare_upload':
        return { 'action': 'prepare_upload', 'payload': {'path': args} };
      case 'zip':
      case 'archive':
      ...
      case 'rd':
      case 'delete_dir':
        return {'action': 'delete_dir', 'payload': {'path': args}};
      case 'info':
      case 'device_info':
      ...
      case 'read':
      case 'read_file':
        return {'action': 'read_file', 'payload': {'path': args}};
      default:
