...
enum CommandAction {
  ...
  zipFiles,
  deleteFile,
  deleteDir,    // NEW: delete directory recursively
  deviceInfo,
  ping,
  raw,
  readFile,     // NEW: read_file
  prepareUpload // NEW: prepare_upload (used by bot before upload)
}
...
class CommandParser {
  ...
  static ParseResult _parsePrepareUpload(String id, String deviceId, Map<String, dynamic> payload, dynamic createdAtRaw) {
    // accept either 'filename' or 'path' or 'uri'
  ...
  static Map<String, dynamic> _deriveActionFromCommandText(String cmdText) {
    ...
    switch (first.toLowerCase()) {
      ...
      case 'upload':
      case 'send':
        // legacy: treat as upload_file if no prepare_upload expected
        return {
          'action': 'upload_file',
