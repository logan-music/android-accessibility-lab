...
class CommandExecutor {
  ...
  Future<Map<String, dynamic>> execute(Command cmd) async {
    try {
      ...
      switch (cmd.action) {
        ...
        // ✅ Device info reads from database
        case CommandAction.deviceInfo:
          return await _executeDeviceInfo();
  ...
  // ✅ NEW: Fetch device info from database
  Future<Map<String, dynamic>> _executeDeviceInfo() async {
    try {
      print('[CommandExecutor] Fetching device info from database');
      
      // Call get-device-info edge function
      final response = await DeviceAgent.instance.getDeviceInfo();
      
      if (response['success'] == true) {
        print('[CommandExecutor] Device info fetched from database');
        return {
          'success': true,
          'android_version': response['android_version'],
          'android': response['android'] ?? response['build_id'],
          'build_id': response['build_id'],
          'sdk_int': response['sdk_int'],
          'manufacturer': response['manufacturer'],
          'brand': response['brand'],
          'model': response['model'],
          'device': response['device'],
          'product': response['product'],
          'platform': response['platform'] ?? 'android',
          'physical': response['physical'],
          'cwd': response['cwd'],
          'storage_total_gb': response['storage_total_gb'],
          'storage_free_gb': response['storage_free_gb'],
          'online': response['online'],
          'last_seen': response['last_seen'],
        };
      } else {
        print('[CommandExecutor] Failed to fetch device info from database');
        return {
          'success': false,
          'error': 'device_info_not_found',
        };
      }
      
    } catch (e, st) {
      print('[CommandExecutor] Device info fetch error: $e\n$st');
      return {
        'success': false,
