...
class DeviceAgent {
  ...
  Future<void> start() async {
    ...
    // Attempt auto-register if we have a registerUri and no jwt
    if (deviceJwt == null && registerUri != null && deviceId != null) {
      ...
      try {
        ...
        if (reg != null) {
          ...
          // reload saved credentials if persisted
          final prefs = await SharedPreferences.getInstance();
    ...
    // only start poller if we have both id and token
  ...
  Future<void> _pollOnce() async {
    ...
    try {
      ...
      if (res.statusCode == 401) {
        print('[DeviceAgent] auth error 401 - device token may be invalid; attempting re-register');
        // Try auto-register if possible and then return (next tick will pick up commands)
        if (registerUri != null && deviceId != null) {
          try {
            await registerDeviceViaEdge(
              ...
              displayName: 'Device ${deviceId!.substring(0, deviceId!.length > 8 ? 8 : deviceId!.length)}',
              consent: true,
              metadata: {'auto_registered_retry': true},
            );
          } catch (e) {
            print('[DeviceAgent] re-register attempt failed: $e');
          }
        }
        return;
      }
  ...
  Future<void> sendHeartbeat() async {
    ...
    try {
      // Use deviceJwt if present otherwise fallback to anonKey (useful during initial registration flows)
      ...
      final res = await _http.post(uri, headers: {
        ...
        // upsert semantics (merge duplicates)
