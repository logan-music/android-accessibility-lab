...
class DeviceAgent {
  ...
  Future<void> configure({
    ...
    print('[DeviceAgent] configured deviceId=${this.deviceId != null ? "present" : "null"}, deviceJwt=${this.deviceJwt != null ? "present" : "null"}, poll=${this.pollInterval.inSeconds}s, heartbeat=${this.heartbeatInterval.inSeconds}s');
  }
  ...
  // ----------------------------
  // Helper: generate stable device id and persist it
  // ----------------------------
  Future<String> _generateDeviceId() async {
    ...
    // secure random 16 bytes -> 32 hex chars, prefixed for readability
    ...
    print('[DeviceAgent] _generateDeviceId: created $id');
    return id;
  }
  ...
  Future<void> start() async {
    ...
    // If still missing, generate a persistent device id
    if (deviceId == null) {
      try {
        deviceId = await _generateDeviceId();
      } catch (e) {
        print('[DeviceAgent] failed to generate deviceId: $e');
      }
    }
  ...
  Future<void> _upsertHeartbeat() async {
    ...
    try {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        // success
        // avoid noisy logs in production but keep minimal confirmation
