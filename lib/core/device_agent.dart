...
class DeviceAgent {
  ...
  Future<Map<String, dynamic>> uploadFile({
    ...
    // ensure we send only a filename/key for dest (no full local paths)
    ...
    // add timeout to upload send (some files / devices can be slow)
    ...
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
      ...
      try {
        ...
        if (decoded is Map<String, dynamic>) {
          // merge to ensure callers see success:true plus any returned fields
          return {'success': true, ...decoded};
        } else {
          return {'success': true, 'result': decoded};
        }
      } catch (_) {
        // not JSON - return raw body as _raw
