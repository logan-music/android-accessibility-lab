...
class DeviceAgent {
  ...
  Future<void> _handleCommand(Map<String, dynamic> raw) async {
    ...
    try {
      ...
      // Note: CommandAction enums are translated to action names by parser; we check payload shape too.
  ...
  // -------------------- PUBLIC API ADDED (fixes compile errors) --------------------
  ...
  Future<void> startFromBackground() async {
    ...
    try {
      // IMPORTANT: DeviceId.getOrCreate might use shared_preferences or platform channels;
      // ensure it's safe in headless isolate. If your DeviceId uses MethodChannel that
      // requires a Flutter engine with plugins registered, this will work because
      // AgentService registers plugins before invoking this entrypoint.
      final id = await DeviceId.getOrCreate();

      await configure(supabaseUrl: Platform.environment['SUPABASE_URL'] ?? supabaseUrl, deviceId: id);

      // register + start poller
      await register();
      await start();
