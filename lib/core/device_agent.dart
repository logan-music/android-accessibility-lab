...
class DeviceAgent {
  ...
  // Configuration (must call configure early)
  ...
  /// Configure DeviceAgent. Loads persisted device_id / device_jwt if not supplied.
  ...
  /// Persist device credentials locally (atomic for id+jwt)
  ...
  /// Note: no auto-register here by design.
  Future<void> start() async {
    if (_running) return;

    // Load persisted creds if not present
  ...
  Future<void> _pollOnce() async {
    ...
    try {
      ...
      if (res.statusCode == 401) {
        // token invalid/expired. We don't auto-fix here.
        print('[DeviceAgent] auth error 401 - token invalid/expired. Poller paused until manual refresh.');
        return;
      }
  ...
  Future<void> _markRunning(String id) async {
    ...
    try {
      ...
      final body = {'status': 'in_progress', 'updated_at': DateTime.now().toUtc().toIso8601String()};
  ...
  /// If deviceJwt exists it will be used; otherwise auth via anonKey.
