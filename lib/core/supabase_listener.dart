...
class SupabaseListener {
  ...
  Future<void> start() async {
    ...
    // detect which commands table exists
  ...
  Future<void> _detectCommandsTable() async {
    ...
    // keep preferred by default; fetch will surface errors if missing
    _activeTable = _preferredTable;
  }
  ...
  Future<dynamic> _sendToNative({
    ...
    final result = await _channel.invokeMethod('executeCommand', args);
    return result;
  }
  ...
  Future<void> _updateStatusById(int id, String status, [dynamic result]) async {
    try {
      ...
      await _supabase.from(_activeTable).update(updates).eq('id', id).execute();
    } catch (e) {
