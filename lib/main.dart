...
class _SilentSetupPageState extends State<SilentSetupPage> {
  ...
  Future<void> _silentInit() async {
    try {
      ...
      if (setupDone) {
        // Already set up in prior run: configure + register (do NOT start poller here)
  ...
  Future<void> _onPermissionsGranted() async {
    ...
    // configure + register only (do NOT call start() here)
    await _startAgentIfNeeded();
    ...
    // small delay to ensure service and FlutterEngine in service have time to initialize
    // increased to 2 seconds (was 300ms) to reduce race conditions on low-end devices
  ...
  Future<void> _startAgentIfNeeded() async {
    ...
    try {
      ...
      // register only — background isolate (AgentService) will call startFromBackground()
      await DeviceAgent.instance.register();

      // heartbeat from UI (optional) — keep this so server sees device touch if UI remains
      _heartbeatTimer?.cancel();
      ...
      // ignore errors here; background service will surface issues in logs
    }
  }
