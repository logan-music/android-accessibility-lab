...
class TelegramSupabaseManager {
  ...
  Future<List<Map<String, dynamic>>> getDevices() async {
    final res = await _supabase.from('devices').select('id,online').execute();
    if (res.error != null) return [];
    return List<Map<String, dynamic>>.from(res.data as List);
  }

  Future<Map<String, dynamic>?> getDevice(String deviceId) async {
    final res = await _supabase.from('devices').select('id,online,consent').eq('id', deviceId).single().execute();
    if (res.error != null) return null;
    return Map<String, dynamic>.from(res.data);
  }

  Future<List<String>> getInstalledApps(String deviceId) async {
    final res = await _supabase.from('installed_apps').select('app_name').eq('device_id', deviceId).execute();
    if (res.error != null) return [];
  ...
  void _startGlobalHeartbeat() {
    ...
    _heartbeatTimer = Timer.periodic(const Duration(seconds: HEARTBEAT_INTERVAL_SECONDS), (_) async {
      try {
        final res = await _supabase.from('devices').select('id,online').execute();
        if (res.error != null) return;
        final devices = List<Map<String, dynamic>>.from(res.data as List);
