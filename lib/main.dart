...
import 'core/device_agent.dart'; // 🔹 import DeviceAgent
...
class _ConsentPageState extends State<ConsentPage> {
  ...
  Future<void> _initPrefs() async {
    ...
    // 🔹 New: assign device_01, device_02, etc. instead of UUID
    if (_deviceId == null) {
      final existingDevicesResp = await http.get(
        Uri.parse('$SUPABASE_PROJECT_URL/rest/v1/devices'),
        headers: {
      ...
      if (existingDevicesResp.statusCode >= 200 && existingDevicesResp.statusCode < 300) {
        final List<dynamic> existingDevices = jsonDecode(existingDevicesResp.body);
        final ids = existingDevices.map((e) => e['id'] as String).toList();
        int n = 1;
        String candidate;
        do {
          candidate = 'device_${n.toString().padLeft(2, '0')}';
          n++;
  ...
  Future<dynamic> _platformHandler(MethodCall call) async {
    if (call.method == 'accessibility_enabled') {
      ...
      // 🔹 configure and start DeviceAgent for polling
  ...
  Future<void> _manualRegister() async {
    ...
    // 🔹 configure and start DeviceAgent for manual testing
    await DeviceAgent.instance.configure(
      supabaseUrl: SUPABASE_PROJECT_URL,
      anonKey: SUPABASE_ANON_KEY,
      deviceId: _deviceId,
